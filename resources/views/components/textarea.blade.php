@props(['label' => null, 'error' => null, 'hint' => null, 'required' => false, 'rows' => 3])
<div>
    @if($label)
    ...
    @endif
    <textarea rows="{{ $rows }}" {{ $attributes->merge(['class' => 'w-full rounded-lg border bg-white px-3.5 py-2.5 text-sm text-slate-800 transition duration-150 outline-none placeholder:text-slate-400 disabled:bg-slate-50 disabled:text-slate-500 ' . ($error ? 'border-rose-300 bg-rose-50/20 text-rose-900 focus:border-rose-500 focus:ring-3 focus:ring-rose-500/15' : 'border-slate-300 hover:border-slate-400 focus:border-emerald-600 focus:ring-3 focus:ring-emerald-600/15')]) }}></textarea>
    @if($error)
